...
- **Connected mode** — Supabase for auth + persistence, Anthropic for real AI, and optional provider keys for GitHub pushes, deployments, payments, and analytics.

## Prerequisites

- **Node.js 20+** (22 recommended) and npm 10+
- A [Supabase](https://supabase.com) project (free tier works) — connected mode only
- An [Anthropic API key](https://console.anthropic.com) for real AI generation
- Optional: [NVIDIA](https://build.nvidia.com), [Paystack](https://dashboard.paystack.com), [Flutterwave](https://app.flutterwave.com), [PostHog](https://posthog.com) accounts
...
ANTHROPIC_API_KEY=<anthropic key>                   # real AI generation
```
...
- **AI chat**: send a message — with `ANTHROPIC_API_KEY` set you get real streaming responses
...
| AI chat returns a canned demo response | `ANTHROPIC_API_KEY` not set |
