...
export interface NvidiaCompletion {
  text: string;
  /**
   * Reasoning models stream their scratchpad in a separate
   * `reasoning_content` field and only then emit `content`. Kept apart
   * from `text` (it is thinking, not an answer) but never discarded: a
   * model cut short mid-thought has produced nothing in `content`, and
   * the work it did do is the only thing standing between the caller and
   * a total failure.
   */
  reasoning: string;
  usage: NvidiaUsage;
