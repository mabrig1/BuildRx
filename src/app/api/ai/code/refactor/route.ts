...
import { refactorCode } from "@/lib/coding/ai";
import { refactorCodeSchema } from "@/lib/validations/coding";

export const maxDuration = 60;

/**
 * POST /api/ai/code/refactor — refactors a code snippet per a
 * free-text instruction. Body: { code, language?, instruction,
...
export async function POST(request: Request) {
  ...
  const parsed = refactorCodeSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Couldn't refactor this code." },
      { status: 502 }
    );
  }
}
